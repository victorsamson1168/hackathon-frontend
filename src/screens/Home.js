...
const BorderLinearProgress = styled(LinearProgress)(({ theme, points }) => ({
  ...
  [`&.${linearProgressClasses.colorPrimary}`]: {
    ...
    borderColor:
      points <= 400 ? "#ff2424" : points >= 800 ? "#31d11b" : "#ffa361",
    borderWidth: 1,
    borderStyle: "solid",
  },
  [`& .${linearProgressClasses.bar}`]: {
    borderRadius: 5,
    backgroundColor:
      points <= 400 ? "#ff2424" : points >= 800 ? "#31d11b" : "#ffa361", //#ff963b
  },
}));
...
function Home() {
  ...
  const [lastRadarMonth, setlastRadarMonth] = React.useState("March");
  const [lastRadarMonthIndex, setlastRadarMonthIndex] = React.useState(
    moment().month()
  );
  ...
  const getYourColleagues = async () => {
    try {
      ...
      if (response.status === 200 && response.data.result) {
        ...
        setContrastedWith(
          team_mates[0].first_name + " " + team_mates[0].last_name
        );
        createRadarData(team_mates[0].uuid, uuid, moment().month());
      }
    } catch (error) {
      console.log("catch err", error);
      setRadarLoading(false);
  ...
  const getCurrentYearScore = async () => {
    ...
    try {
      ...
      const response = await API.getThisYearScore(uuid, moment().year());
      if (response.status === 200 && response.data.result) {
  ...
  const RadarMonthChangeHandler = async (eve) => {
    console.log("e", eve.target.value);

    let upcoming_month = months;
    // upcoming_month = upcoming_month.filter((month, index) => index >= 3);
    if (upcoming_month.includes(eve.target.value)) {
      // setScoreFound(405);
      // setScores([]);
      setlastRadarMonthIndex(months.indexOf(eve.target.value) + 1);
      setlastRadarMonth(eve.target.value);
    }
    createRadarData(
      contrastedTo.uuid,
      uuid,
      months.indexOf(eve.target.value) + 1
    );
  };

  const createRadarData = async (othersId, myId, month_idx) => {
    let sample_arr = [];
    let month = month_idx;
  ...
  return (
    <div style={{ width: "100%", height: "100%" }}>
      <Paper elevation={3} sx={{ ...styles.singleScoreCard, marginBottom: 2 }}>
        {myScoreLoading ? (
          <LinearProgressAnimated sx={{ marginTop: -1, marginBottom: 1 }} />
        ) : null}
        <Grid item xs={12}>
          <Box
            sx={{
                ...
                <Chip
                  icon={
                    scoreFound == 200 ? (
                      ...
                      <ReportProblemOutlinedIcon sx={{ height: 18 }} />
                    ) : (
                ...
                <Box sx={styles.singleScoreCardBox}>
                  ...
                  >
                    <Typography sx={{ fontSize: 12 }}>
                      {"Contribution"}
                      {` (${item.PMP_QUETION.eval_percentage}%)`}
                    </Typography>
                  </Box>
                  <Box>
        ...
        <Grid item xs={4}>
          <Paper elevation={6} sx={styles.singleScoreCard}>
              ...
              <Box sx={{ minWidth: 120 }}>
                <FormControl fullWidth>
                  ...
                  <Select
                    labelId="demo-simple-select-label"
                    id="demo-simple-select"
                    value={lastRadarMonth}
                    label="Month"
                    onChange={RadarMonthChangeHandler}
                    size="small"
                    sx={{ height: 27, fontSize: 14, fontWeight: "bold" }}
                  >
                    {months.map((item, index) => {
            ...
            >
              ...
              >
                <Typography
                  sx={{ width: 200, fontSize: 15, fontWeight: "bold" }}
                >
                  {"Specific to area"}
                </Typography>
              ...
              {radarArray.map((item, index) => (
                ...
                >
                  <Typography sx={{ width: 200, fontSize: 13 }}>
                    {item.que_text}
                  </Typography>
                  <Typography
                    sx={{ width: 80, fontSize: 13, textAlign: "left" }}
                  >
                    {item.A}
                  </Typography>
                  <Typography
      ...
      <Grid container spacing={2}>
        <Grid item xs={8} marginTop={5}>
          <Paper
            elevation={3}
            sx={{ ...styles.singleScoreCard, marginBottom: 2 }}
          >
            <Typography sx={{ fontSize: 16, fontWeight: "bold" }}>
              Reviews / Comments
            </Typography>
        ...
        <Grid item xs={4} marginTop={5}>
          <Paper
            elevation={3}
            sx={{ ...styles.singleScoreCard, marginBottom: 2, padding: 0 }}
          >
              ...
              <CardContent>
                ...
                <TooltipNormal title={manager} placement="top">
                  <Avatar
                    alt={manager}
                    src={managerPic}
                    sx={styles.avatarImgStyleManager}
                  />
                </TooltipNormal>
              </CardContent>
            </div>
          ...
          >
            <div style={{ display: "flex" }}>
              <CardContent>
                <Typography sx={{ ...styles.boldTxt, color: "#ffffff" }}>
                  Redeemable Points
                </Typography>
                <Box
                  sx={{
                ...
                >
                  <Paper
                    elevation={3}
                    sx={{ height: "auto", padding: 1, borderRadius: 10 }}
                  >
